...
public class BattleController {
    ...
    @GetMapping
    public ResponseEntity<List<Battle>> findAll() { return new ResponseEntity<>(service.findAll(), HttpStatus.OK); }
    ...
    public ResponseEntity<?> update(@PathVariable String id, @RequestBody Battle battle) {
        Battle battleAModifier = service.findById(id);
        if (battleAModifier == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
